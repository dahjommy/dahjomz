...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_user'])) {
        ...
        if (empty($validationErrors)) {
            ...
            if ($result['success']) {
                ...
                $errors[] = 'Failed to add user: ' . $result['message'];
            }
        } else {
    ...
    if (isset($_POST['delete_user'])) {
        ...
        if ($userId === $user['id']) {
            ...
            $result = $userObj->delete($userId);
            if ($result['success']) {
                setAlert('User deleted successfully!', 'success');
            } else {
                setAlert('Failed to delete user: ' . $result['message'], 'danger');
            }
        }
        redirect('users.php');
    }
            ...
            <div class="content">
                ...
                <div class="page-header">
                    <div class="page-header-content">
                        <h1 class="page-title"><i class="bi bi-person-badge"></i> User Management</h1>
                        <p class="page-subtitle">Manage system users and their access levels</p>
                    </div>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addUserModal" style="position: relative; z-index: 1;">
                        ...
                        <table id="usersTable" class="table table-striped table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-<?php echo $usr['role'] === 'admin' ? 'danger' : 'warning'; ?>">
                                            <?php echo ucfirst($usr['role']); ?>
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge bg-<?php echo $usr['status'] === 'active' ? 'success' : 'secondary'; ?>">
                                            <?php echo ucfirst($usr['status'] ?? 'active'); ?>
                                        </span>
                                    </td>
                                    <td>
                                        <div class="action-buttons">
                                            <button class="btn btn-sm btn-info text-white view-user" 
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <button class="password-toggle" type="button" onclick="togglePassword('add-password', this)">
                                        <i class="bi bi-eye"></i>
                                    </button>
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.view-user').click(function() {
                ...
                $('#viewUserContent').html('<div class="text-center p-3"><div class="spinner-border text-primary" role="status"></div></div>');
                
                $.get('ajax_handler.php', { action: 'getUser', id: userId }, function(response) {
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="info-group">
                                        <label>User ID</label>
                                        <div class="value">${usr.id}</div>
                                    </div>
                                    <div class="info-group">
                                        <label>Username</label>
                                        <div class="value">${usr.username}</div>
                                    </div>
                                    <div class="info-group">
                                        <label>Email</label>
                                        <div class="value">${usr.email || 'N/A'}</div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="info-group">
                                        <label>Full Name</label>
                                        <div class="value">${usr.fullName || 'N/A'}</div>
                                    </div>
                                    <div class="info-group">
                                        <label>Role</label>
                                        <div class="value">
                                            <span class="badge bg-${usr.role === 'admin' ? 'danger' : 'warning'}">
                                                ${usr.role ? usr.role.charAt(0).toUpperCase() + usr.role.slice(1) : 'N/A'}
                                            </span>
                                        </div>
                                    </div>
                                    <div class="info-group">
                                        <label>Status</label>
                                        <div class="value">
                                            <span class="badge bg-${usr.status === 'active' ? 'success' : 'secondary'}">
                                                ${usr.status ? usr.status.charAt(0).toUpperCase() + usr.status.slice(1) : 'Active'}
                                            </span>
                                        </div>
                                    </div>
                                </div>
            ...
            $('.edit-user').click(function() {
                const userId = $(this).data('id');
                $('#editUserContent').html('<div class="text-center p-3"><div class="spinner-border text-primary" role="status"></div></div>');
                ...
                $.get('ajax_handler.php', { action: 'getUser', id: userId }, function(response) {
                    ...
                    if (data.success) {
                        ...
                        const html = `
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" value="${usr.username}" disabled>
                                </div>
                                <div class="col-md-6 mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <button class="password-toggle" type="button" onclick="togglePassword('edit-password', this)">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                    </div>
                                </div>
                        ...
                        $('#editUserContent').html('<div class="alert alert-danger">Failed to load user details</div>');
                    }
                });
            });
            ...
            $('#editUserForm').submit(function(e) {
                e.preventDefault();
                const formData = $(this).serialize() + '&action=updateUser';
